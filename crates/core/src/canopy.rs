...
fn emit_canopy(app: &AppCtx, msg: &str) { let _ = app.emit("cdp:log", msg); }
...
async fn run_competition_canopy(app: &AppCtx, database: &db::Db, req: &CompetitionCanopyRequest) -> CompetitionResult {
...
pub async fn search_keywords_canopy(app: AppCtx, request: KeywordSearchCanopyRequest) -> KeywordSearchResponse {
...
pub async fn mine_competitor_reviews(app: AppCtx, request: ReviewMiningRequest) -> ReviewMiningResult {
    let database = app.db.as_ref();
    let client = match CanopyClient::new(&request.canopy_api_key) {
        Ok(c) => c,
...
pub async fn analyze_comp_authors(app: AppCtx, request: AuthorAnalysisRequest) -> AuthorAnalysisResult {
    let database = app.db.as_ref();
    let client = match CanopyClient::new(&request.canopy_api_key) {
        Ok(c) => c,
...
pub async fn deep_category_analysis(app: AppCtx, canopy_api_key: String, category_path: String, node_id: String, store: String) -> DeepCategoryResult {
    let client = match CanopyClient::new(&canopy_api_key) {
        Ok(c) => c,
