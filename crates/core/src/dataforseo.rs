...
//   /keywords_data/dataforseo_trends/explore/live                   — trend series for keywords
//   /serp/google/autocomplete/live/advanced                         — seed expansion
//   /dataforseo_labs/amazon/related_keywords/live                   — Amazon related keywords with volume
//   /dataforseo_labs/amazon/bulk_search_volume/live                 — Amazon volume for many keywords
//   /dataforseo_labs/amazon/ranked_keywords/live                    — keywords a known ASIN ranks for
//   /dataforseo_labs/amazon/product_competitors/live                — competitor ASINs for an ASIN
//   /dataforseo_labs/amazon/product_keyword_intersections/live      — keywords shared by several ASINs
...
/// Read a keyword + volume pair out of one of the several shapes DataForSEO Labs
/// endpoints use (flat, `keyword_info`, or `keyword_data.keyword_info`).
...
impl DataForSeoClient {
            ...
            for task in tasks_arr {
                if let Some(result_arr) = task["result"].as_array() {
                    for result in result_arr {
                        results.extend(parse_keyword_items(result));
                    }
                }
            }
        }
    ...
    pub async fn amazon_product_keyword_intersections(&self, asins: &[String], limit: u32) -> Result<Vec<AmazonKeyword>, String> {
        let asins = normalize_keyword_list(asins);
    ...
    /// Pull trend deltas from the DataForSEO Trends endpoint — percentage change
    /// between the first and last point of each keyword's series.
