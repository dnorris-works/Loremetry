...
pub async fn find_categories_for_story(app: AppCtx, request: FindCategoriesRequest) -> GenreResult {
    ...
    // Get ranked genres
    let genre_terms: Vec<(String, u8)> = db::get_genre_rankings(&database.pool, &request.story_id, &request.store)
        .await
        .unwrap_or_default()
        .into_iter()
    ...
    let result = match_categories_by_store(&app, &database, &request.story_id, &request.store, &base_description, &genre_terms, &request.provider, &request.api_key, &request.model).await;
    ...
    let _ = db::save_document(&database.pool, &request.story_id, "category_finder", &report).await;
...
pub async fn match_categories_for_story(app: AppCtx, request: FindCategoriesRequest) -> GenreResult {
    ...
    let rankings = db::get_genre_rankings(&database.pool, &request.story_id, "Kindle")
        .await
        .unwrap_or_default();
...
pub async fn verify_mapped_categories(app: AppCtx, db: &db::Db, request: VerifyMappedRequest) -> Result<GenreResult, ()> {
    let canopy_key = {
    ...
    let rankings = match db::get_genre_rankings(&db.pool, &request.story_id, &request.store).await {
        Ok(r) => r,
    ...
    let _ = db::save_document(&db.pool, &request.story_id, "mapped_categories", &md).await;
    for g in &rankings {
        for p in &g.kdp_paths {
            let _ = db::upsert_kdp_path(&db.pool, &g.genre, p, &request.store, "category_analyzer", true).await;
        }
    }
...
/// discoverability.
pub(crate) async fn rank_by_discoverability(app: &AppCtx, store: &str, qualifying: Vec<(String, u8, Vec<String>)>, canopy_api_key: &str) -> Vec<QualifiedCategory> {
...
pub(crate) async fn match_categories_by_store(
    app: &AppCtx, database: &db::Db, folder: &str, store: &str,
    base_description: &str, genre_terms: &[(String, u8)],
    provider: &str, api_key: &str, model: &str,
) -> StoreMatchResult {
    let thresholds = db::load_analysis_thresholds(&database.pool).await;
    ...
    for (genre_name, genre_conf) in genre_terms {
        ...
        let candidates = db::search_kdp_categories(&database.pool, store, std::slice::from_ref(genre_name), 100)
            .await;
    ...
    for (genre_name, genre_conf, candidates) in genre_candidates {
        let picks = if let Some(batch) = batch_picks.get(&genre_name) {
            ...
            match ai_match_from_catalog(app, folder, provider, api_key, model, &desc, &candidates, 2).await {
                Ok(p) => p,
    ...
    if !ranked_paths.is_empty() {
        ...
        let _ = db::replace_category_results(&database.pool, folder, store, top_genre.as_deref(), &rows).await;
    }
    emit(app, &format!(
...
/// Score every ranked genre's candidate list in a single AI call.
/// Returns picks keyed by genre name; genres absent from the response fall back
/// to a per-genre call at the call site.
#[allow(clippy::too_many_arguments)]
