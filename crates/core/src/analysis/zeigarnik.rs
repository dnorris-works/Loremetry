...
pub async fn analyze_zeigarnik_for_story(app: AppCtx, request: ZeigarnikRequest) -> GenreResult {
    ...
    let mut chapter_texts: Vec<String> = Vec::with_capacity(chapters.len());
    let mut chapter_rows: Vec<db::ZeigarnikChapterRow> = Vec::with_capacity(chapters.len());

    for (i, chapter) in chapters.iter().enumerate() {
        let fname = documents::chapter_display_name(chapter);
        let raw = &chapter.content;
        ...
        emit(&app, &format!("  [{}/{}] {} — {} words, {} question(s), {} ending",
            i + 1, chapters.len(), fname, word_count, questions.len(), ending_type));
