...
pub struct AnalyzeStoryRequest {
    ...
    /// Per-report effective models from auto-pick + overrides (wins over `model`).
    #[serde(default)]
    pub model_by_report:   std::collections::HashMap<String, String>,
}

/// Resolve model for a report: model_by_report wins, else default `model`.
pub fn story_model_for<'a>(request: &'a AnalyzeStoryRequest, report_id: &str) -> &'a str {
    request
        .model_by_report
        .get(report_id)
        .map(|s| s.as_str())
