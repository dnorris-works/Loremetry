...
use super::{emit, err, story_model_for, GenreResult, FolderRequest, AnalyzeStoryRequest};
use crate::db;
...
async fn analyze_story_inner(app: AppHandle, request: AnalyzeStoryRequest) -> GenreResult {
    ...
    let has_model = !request.model.is_empty()
        || request.model_by_report.values().any(|m| !m.is_empty());
    if !has_model { return err("No model selected. Go to Settings (set a default or run Auto-pick)."); }

    let database = app.state::<db::Db>();
    let run_ts = if request.run_time.is_empty() { chrono::Utc::now().to_rfc3339() } else { request.run_time.clone() };

    let m_summaries = story_model_for(&request, "chapter_summaries");
    let m_genre = story_model_for(&request, "genre_analysis");
    let m_ranking = story_model_for(&request, "genre_ranking");
    let m_tropes = story_model_for(&request, "trope_market_fit");
    let m_categories = story_model_for(&request, "kdp_categories");
    let m_keywords = story_model_for(&request, "kdp_keywords");
    let m_bisac = story_model_for(&request, "bisac_classification");
    let m_search = story_model_for(&request, "mi_search_terms");
    let m_discovery = story_model_for(&request, "discovery_keywords");
    ...
    emit(&app, "Step 1: Chapter summaries...");
    {
        ...
        let (done, skipped) = phase1_summaries(&app, &database, &chapters, &request.folder, &request.provider, &request.api_key, m_summaries).await;
    ...
    if genre_data_existing.is_none() {
        ...
        if summaries.is_empty() { return err("No chapter summaries available."); }
        let r = phase2_analyze(&app, &database, &request.folder, &summaries, &request.provider, &request.api_key, m_genre).await;
        if !r.success { return err(&r.error); }
    } else {
    ...
    let ranked: Vec<RankedGenre> = {
        ...
        let ai_ranked = match ai_rank_genres(&database, &request.provider, &request.api_key, m_ranking, &description, &master_list).await {
            Ok(r) => r,
            Err(e) => return err(&format!("Genre ranking failed: {}", e)),
        };
    ...
    // ── Step 3b: Trope Extraction & Market Fit ─────────────────────────────
    emit(&app, "Step 3b: Trope Extraction & Market Fit...");
    {
        let tr = super::tropes::run_trope_market_fit(
            &app, &database, &request.folder,
            &request.provider, &request.api_key, m_tropes, &run_ts,
        ).await;
        if !tr.success {
            emit(&app, &format!("  ⚠ Trope Extraction: {}", tr.error));
            // Non-fatal: continue the Shared pipeline (genre ranking already succeeded)
        }
    }
    if crate::is_cancelled() { return err("Cancelled."); }
    ...
    for (store, label, top_cats) in [
        ...
        let result = match_categories_by_store(&app, &database, &request.folder, store, &base_description, &genre_terms, &request.provider, &request.api_key, m_categories).await;
    ...
    emit(&app, "Step 5: Generating competition search terms...");
    {
        match generate_mi_search_terms(&database, &request.provider, &request.api_key, m_search, &genre_data).await {
            Ok(keywords) => {
                let conn = database.0.lock().unwrap();
                let _ = db::save_mi_search_terms(&conn, &request.folder, &keywords);
                let rendered = render_search_terms(&keywords);
    ...
    let bisac_section: String = {
        ...
        let ebook_desc = format!("{}\n\n{}", genre_data.industry_ebook, genre_data.genre_signals);
        let ebook_picks = ai_pick_bisac(&database, &request.provider, &request.api_key, m_bisac, &ebook_desc, &bisac_master).await.unwrap_or_default();
        {
            let conn = database.0.lock().unwrap();
            let rows: Vec<(String, String, u8, String)> = ebook_picks.iter().map(|(c, h, cf, r)| (c.clone(), h.clone(), *cf, r.clone())).collect();
            let _ = db::replace_bisac_classifications(&conn, &request.folder, "ebook", &rows);
        ...
        let print_picks = if same_as_ebook {
            ...
            let print_desc = format!("{}\n\n{}", genre_data.industry_print, genre_data.genre_signals);
            let picks = ai_pick_bisac(&database, &request.provider, &request.api_key, m_bisac, &print_desc, &bisac_master).await.unwrap_or_default();
            let conn = database.0.lock().unwrap();
    ...
    {
        let res = call_keyword_optimizer_with_pool(&database, &request.provider, &request.api_key, m_keywords, &genre_data, &genre_data.genre_signals, &keyword_pool).await;
    ...
    let discovery_entries: Vec<db::DiscoveryKeywordEntry> = {
        let res = generate_discovery_keywords(&database, &request.provider, &request.api_key, m_discovery, &genre_data).await;
...
pub struct CraftPipelineRequest {
    ...
    #[serde(default)]
    pub model_prose:      String,           // override for prose / creative copy
    /// Per-report effective models from auto-pick + overrides (wins over slot fields).
    #[serde(default)]
    pub model_by_report:  std::collections::HashMap<String, String>,
...
fn craft_model_for(request: &CraftPipelineRequest, report_id: &str, legacy: &str) -> String {
    if let Some(m) = request.model_by_report.get(report_id) {
        if !m.is_empty() {
            return m.clone();
        }
    }
...
async fn run_craft_pipeline_inner(app: AppHandle, request: CraftPipelineRequest) -> GenreResult {
    ...
    // Resolve per-report models (model_by_report wins; else legacy slots; else default)
    let model_summaries = craft_model_for(&request, "chapter_summaries", &request.model_summaries);
    let model_continuity = craft_model_for(&request, "continuity_check", &request.model_continuity);
    let model_sdt = craft_model_for(&request, "show_dont_tell", &request.model_sdt);
    let model_ai_isms = craft_model_for(&request, "ai_isms", &request.model_ai_isms);
    let model_prose = craft_model_for(&request, "blurb_builder", &request.model_prose);
    let model_dialogue = craft_model_for(&request, "dialogue_voice", &request.model_sdt);
    let model_beta = craft_model_for(&request, "ai_beta_reader", &request.model_prose);
    let model_ad = craft_model_for(&request, "ad_copy_hook_pack", &request.model_prose);
    let model_cliff = craft_model_for(&request, "cliffhanger_score", &request.model_summaries);
    let model_hook = craft_model_for(&request, "hook_strength", &request.model_summaries);
    let model_pace = craft_model_for(&request, "pacing_curve", &request.model_summaries);
    ...
    let needs_ai = request.selected.iter().any(|s| {
        s == "chapter_summaries"
            ...
            || matches!(s.as_str(), "ai_beta_reader" | "cliffhanger_score" | "hook_strength" | "pacing_curve" | "blurb_builder" | "ad_copy_hook_pack")
    });

    let has_model = !request.model.is_empty()
        || request.model_by_report.values().any(|m| !m.is_empty())
        || !request.model_summaries.is_empty()
        || !request.model_continuity.is_empty()
        || !request.model_sdt.is_empty()
        || !request.model_ai_isms.is_empty()
        || !request.model_prose.is_empty();
    if needs_ai && (request.api_key.is_empty() || !has_model) {
    ...
    // ── Dialogue & Voice ──────────────────────────────────────────────────
    if request.selected.contains(&"dialogue_voice".to_string()) {
        let dv = super::dialogue_voice::run_dialogue_voice(
            &app, &database, &request.folder,
            &request.provider, &request.api_key, &model_dialogue, &request.bible_path,
        ).await;
        if !dv.success {
            emit(&app, &format!("✗ Dialogue & Voice: {}", dv.error));
            return dv;
        }
        if crate::is_cancelled() { return err("Cancelled."); }
    }
    ...
    for audit_id in super::craft_audits::MANUSCRIPT_AUDITS {
        ...
        let model_craft = craft_model_for(&request, audit_id, &request.model_continuity);
    ...
    if request.selected.iter().any(|s| s == "cliffhanger_score") {
        let r = super::publish_audits::run_cliffhanger_score(
            &app, &database, &request.folder,
            &request.provider, &request.api_key, &model_cliff,
        ).await;
        if !r.success { return r; }
        if crate::is_cancelled() { return err("Cancelled."); }
    }
    if request.selected.iter().any(|s| s == "hook_strength") {
        let r = super::publish_audits::run_hook_strength(
            &app, &database, &request.folder,
            &request.provider, &request.api_key, &model_hook, &request.bible_path,
        ).await;
        if !r.success { return r; }
        if crate::is_cancelled() { return err("Cancelled."); }
    }
    if request.selected.iter().any(|s| s == "pacing_curve") {
        let r = super::publish_audits::run_pacing_curve(
            &app, &database, &request.folder,
            &request.provider, &request.api_key, &model_pace,
        ).await;
        if !r.success { return r; }
        if crate::is_cancelled() { return err("Cancelled."); }
    }
    ...
    if request.selected.iter().any(|s| s == "ad_copy_hook_pack") {
        let r = super::publish_audits::run_ad_copy_hook_pack(
            &app, &database, &request.folder,
            &request.provider, &request.api_key, &model_ad,
        ).await;
        if !r.success { return r; }
        if crate::is_cancelled() { return err("Cancelled."); }
    }
