...
const SEED_TROPE_CONFIG_JSON: &str = include_str!("../data/trope-config.json");
const SEED_MODEL_TASK_PROFILES_JSON: &str = include_str!("../data/model-task-profiles.json");
...
CREATE TABLE IF NOT EXISTS report_types (
    ...
    platforms         TEXT NOT NULL DEFAULT 'kdp,wide',  -- comma-separated: 'kdp', 'wide', 'craft', 'publish', 'marketing'
...
-- Trope Extraction & Market Fit: catalog of known tropes / conventions with
-- genre affinity. Seeded once from trope-config.json (tunable without rebuild).
CREATE TABLE IF NOT EXISTS trope_config (
    key   TEXT PRIMARY KEY,
    value TEXT NOT NULL   -- JSON-encoded
);

-- Auto model-pick: per-report task profiles (seeded from model-task-profiles.json).
CREATE TABLE IF NOT EXISTS model_task_profiles (
    report_id TEXT PRIMARY KEY,
    family    TEXT NOT NULL,
    literary  INTEGER NOT NULL,
...
-- User override of auto-picked model for a report (empty/absent = use auto).
CREATE TABLE IF NOT EXISTS report_model_prefs (
    report_id      TEXT PRIMARY KEY,
    override_model TEXT NOT NULL DEFAULT '',
    updated_at     TEXT NOT NULL
);

-- Last auto recommendations (refreshed when quality_bias / catalog changes).
CREATE TABLE IF NOT EXISTS report_model_auto (
    report_id TEXT PRIMARY KEY,
...
pub fn init(app: &AppHandle) -> Result<Db, String> {
    ...
    seed_trope_config_if_empty(&conn)?;
    seed_model_task_profiles(&conn)?;
...
fn seed_report_types(conn: &Connection) -> Result<(), String> {
    ...
    let rows: &[(&str, &str, &str, &str, &str, i64, i64, i64, i64, &str, &str)] = &[
        ...
        ("trope_market_fit", "Trope Extraction & Market Fit", "Detect manuscript tropes from chapter summaries and compare them to genre/trope taxonomy expectations.", "kdp,wide", "chapter_summaries,genre_analysis", 0, 2500, 0, 1, "genre", "capable"),
        ...
        ("dialogue_voice", "Dialogue & Voice", "Dialogue-to-narrative ratios plus AI flags when major characters' dialogue sounds interchangeable.", "craft", "", 0, 2000, 0, 1, "showDontTell", "capable"),
        ...
        // Marketing platform
        ("blurb_builder", "Blurb Builder", "Back-cover / Amazon description variants plus short-form and BookBub one-liners.", "marketing", "", 0, 3000, 0, 1, "prose", "strong"),
        ("ad_copy_hook_pack", "Ad Copy & Hook Pack", "BookBub headlines, Amazon/Facebook ad hooks, and AMS ad-targeting seed phrases for testing.", "marketing", "", 0, 2500, 0, 1, "prose", "strong"),
    ];
...
fn seed_trope_config_if_empty(conn: &Connection) -> Result<(), String> {
    let count: i64 = conn
        .query_row("SELECT COUNT(*) FROM trope_config", [], |r| r.get(0))
        .map_err(|e| e.to_string())?;
    if count > 0 { return Ok(()); }

    let parsed: serde_json::Value = serde_json::from_str(SEED_TROPE_CONFIG_JSON)
        .map_err(|e| format!("Cannot parse seed trope-config.json: {}", e))?;

    let obj = parsed.as_object().ok_or("trope-config.json must be a JSON object")?;
    for (key, value) in obj {
        conn.execute(
            "INSERT OR IGNORE INTO trope_config (key, value) VALUES (?1, ?2)",
            params![key, value.to_string()],
        ).map_err(|e| e.to_string())?;
    }
    Ok(())
...
fn seed_model_task_profiles(conn: &Connection) -> Result<(), String> {
    conn.execute("DELETE FROM model_task_profiles", [])
        .map_err(|e| e.to_string())?;
    let profiles = crate::model_recommend::parse_profile_seeds(SEED_MODEL_TASK_PROFILES_JSON)?;
    for p in profiles {
        conn.execute(
            "INSERT INTO model_task_profiles
             (report_id, family, literary, reasoning, creative, volume, min_band)
...
pub fn load_model_task_profiles(conn: &Connection) -> Vec<crate::model_recommend::ModelTaskProfile> {
    let mut stmt = match conn.prepare(
        "SELECT report_id, family, literary, reasoning, creative, volume, min_band
         FROM model_task_profiles ORDER BY report_id",
    ) {
        Ok(s) => s,
        Err(_) => {
            return crate::model_recommend::parse_profile_seeds(SEED_MODEL_TASK_PROFILES_JSON)
                .unwrap_or_default();
        }
    };
    let rows = stmt.query_map([], |r| {
        Ok(crate::model_recommend::profile_from_seed(
            crate::model_recommend::ProfileSeed {
                report_id: r.get(0)?,
                family: r.get(1)?,
                literary: r.get(2)?,
                reasoning: r.get(3)?,
    ...
    match rows {
        Ok(iter) => {
            let v: Vec<_> = iter.filter_map(|x| x.ok()).collect();
            if v.is_empty() {
                crate::model_recommend::parse_profile_seeds(SEED_MODEL_TASK_PROFILES_JSON)
                    .unwrap_or_default()
            } else {
                v
            }
        }
        Err(_) => crate::model_recommend::parse_profile_seeds(SEED_MODEL_TASK_PROFILES_JSON)
            .unwrap_or_default(),
    }
}

pub fn replace_report_model_autos(conn: &Connection, rows: &[(&str, &str, &str)]) -> Result<(), String> {
    let now = chrono::Utc::now().to_rfc3339();
    conn.execute("DELETE FROM report_model_auto", []).map_err(|e| e.to_string())?;
    for (report_id, model_id, reason) in rows {
        conn.execute(
            "INSERT INTO report_model_auto (report_id, model_id, reason, scored_at) VALUES (?1, ?2, ?3, ?4)",
            params![report_id, model_id, reason, now],
        )
        .map_err(|e| e.to_string())?;
    }
    Ok(())
...
pub fn load_report_model_autos(conn: &Connection) -> std::collections::HashMap<String, (String, String)> {
    let mut map = std::collections::HashMap::new();
    let mut stmt = match conn.prepare("SELECT report_id, model_id, reason FROM report_model_auto") {
        Ok(s) => s,
        Err(_) => return map,
    };
...
pub fn load_report_model_overrides(conn: &Connection) -> std::collections::HashMap<String, String> {
    let mut map = std::collections::HashMap::new();
    let mut stmt = match conn.prepare(
        "SELECT report_id, override_model FROM report_model_prefs WHERE override_model != ''",
    ) {
        Ok(s) => s,
        Err(_) => return map,
    };
...
pub fn set_report_model_override(conn: &Connection, report_id: &str, model_id: &str) -> Result<(), String> {
    let now = chrono::Utc::now().to_rfc3339();
    conn.execute(
        "INSERT INTO report_model_prefs (report_id, override_model, updated_at) VALUES (?1, ?2, ?3)
         ON CONFLICT(report_id) DO UPDATE SET override_model = excluded.override_model, updated_at = excluded.updated_at",
        params![report_id, model_id, now],
    )
    .map_err(|e| e.to_string())?;
    Ok(())
}

pub fn clear_report_model_override(conn: &Connection, report_id: &str) -> Result<(), String> {
    conn.execute("DELETE FROM report_model_prefs WHERE report_id = ?1", params![report_id])
        .map_err(|e| e.to_string())?;
    Ok(())
}

// ── Trope Extraction & Market Fit config ───────────────────────────────────
...
pub struct TropeCatalogEntry {
    ...
    #[serde(default)]
    pub common_in_genres: Vec<String>,
}

#[derive(Clone, Debug, Default)]
pub struct TropeConfig {
    pub catalog: Vec<TropeCatalogEntry>,
}

/// Load the trope catalog from the database. Falls back to empty catalog if
/// seeding somehow failed rather than erroring the whole analysis out.
pub fn load_trope_config(conn: &Connection) -> TropeConfig {
    let catalog = conn
        .query_row("SELECT value FROM trope_config WHERE key = ?1", params!["catalog"], |r| r.get::<_, String>(0))
        .ok()
        .and_then(|s| serde_json::from_str::<Vec<TropeCatalogEntry>>(&s).ok())
        .unwrap_or_default();
    TropeConfig { catalog }
}
