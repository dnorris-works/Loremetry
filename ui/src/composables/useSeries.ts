...
import type { Series, SeriesResult, SeriesBook } from '../types';
...
async function createSeries(name: string, books: SeriesBook[]): Promise<SeriesResult> {
  const result = await invoke<SeriesResult>('create_series', { request: { name, books } });
  if (result.success) seriesList.value = result.series;
  return result;
}

async function updateSeries(id: number, name: string, books: SeriesBook[], biblePath: string = ''): Promise<SeriesResult> {
